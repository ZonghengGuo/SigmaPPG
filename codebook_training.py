...
from codebook.codebook_engine import evaluate, train_one_epoch, calculate_codebook_usage
from codebook.utils import NativeScalerWithGradNormCount as NativeScaler
from codebook import codebook_model
...
def get_args():
    parser = argparse.ArgumentParser('diffusionFM pre-training script', add_help=False)
    parser.add_argument('--data_path', help='input data path')
    parser.add_argument('--batch_size', default=64, type=int)
    parser.add_argument('--epochs', default=100, type=int)
    parser.add_argument('--save_ckpt_freq', default=20, type=int)
    # Model parameters
    parser.add_argument('--model', default='vqnsp_encoder_base_decoder_3x250x12', type=str, metavar='MODEL',
                        help='Name of model to train')

    parser.add_argument('--codebook_n_emd', default=4096, type=int, metavar='MODEL',
    ...
    parser.add_argument('--use_consistency_loss', action='store_true',
                        help='Enable internal consistency loss (augmentation-based) for codebook training.')
    parser.add_argument('--consistency_weight', default=0.5, type=float,
                        help='Weight for consistency loss in codebook training')
    ...
    parser.add_argument('--reconstruct_phase', action='store_true',
                        help='Enable phase reconstruction in addition to amplitude reconstruction (default: False, amplitude only)')
...
def get_model(args, **kwargs):
    ...
    print(f"Phase Reconstruction Enabled: {args.reconstruct_phase}")  # 新增：显示相位重建状态
...
def main(args):
    ...
    if not all_h5_files:
        raise FileNotFoundError(f"No H5 files found in {data_dir}. Please check your merged file path.")


    print(f"A total of {len(all_h5_files)} H5 file(s) found. Using data segment split (Train: 0.0-0.8, Val: 0.8-1.0).")
    ...
    if dataset_train_list:
        total_train_segments = sum(len(d) for d in dataset_train_list)
        print(f"🎉 CONFIRM: Training dataset loaded a total of {total_train_segments} segments.")
    else:
        print("❌ CONFIRM: Training dataset list is empty (0 segments). Check data path/split logic.")

    datasets_val = [val_files]

    if args.disable_eval:
        print("args.disable_eval is True, use None for val dataset.")
        dataset_val_list = None
        val_ch_names_list = None
    else:
        dataset_val_list, val_ch_names_list = utils.build_pretraining_dataset(
            datasets_val, time_window,
            stride_size=400,
            start_percentage=0.0,
            end_percentage=1.0
        )
        if dataset_val_list:
            total_val_segments = sum(len(d) for d in dataset_val_list)
            print(f"🎉 CONFIRM: Validation dataset loaded a total of {total_val_segments} segments.")
        else:
            print("❌ CONFIRM: Validation dataset list is empty (0 segments). Check data path/split logic.")
    ...
    if True:
        ...
        if not dataset_train_list:
            raise ValueError("dataset_train_list is empty, cannot create Sampler.")
        sampler_train = torch.utils.data.RandomSampler(dataset_train_list[0])

        if dataset_val_list is None or not dataset_val_list:
            sampler_val = None
            sampler_eval_list = []
        else:
            sampler_val = torch.utils.data.SequentialSampler(dataset_val_list[0])
            sampler_eval_list = [sampler_val]

        sampler_train_list = [sampler_train]
