...
from codebook.dataset import ShockDataset
import pickle
...
def build_pretraining_dataset(datasets: list, time_window: list, stride_size=200, start_percentage=0, end_percentage=1):
    ...
    for dataset_list, window_size in zip(datasets, time_window):
        dataset = ShockDataset([Path(file_path) for file_path in dataset_list], window_size, stride_size,
                               start_percentage, end_percentage, load_to_memory=load_to_memory)
