...
from codebook.modeling_finetune import Block, _cfg, PatchEmbed
...
class NeuralTransformerForMaskedPPGModeling(nn.Module):
    def __init__(self, PPG_size=12000, patch_size=100, in_chans=1, out_chans=8, vocab_size=2048, embed_dim=200, depth=12,
        ...
        self.patch_embed_conv = TemporalConv(out_chans=out_chans)

        t_conv1 = math.floor((self.patch_size - 15 + 2 * 7) / 8) + 1
        t_conv2 = math.floor((t_conv1 - 3 + 2 * 1) / 1) + 1
        t_conv3 = math.floor((t_conv2 - 3 + 2 * 1) / 1) + 1
        temporal_conv_output_dim = t_conv3 * out_chans  # 结果应为 256

        if temporal_conv_output_dim != embed_dim:
            self.patch_embed_projection = nn.Linear(temporal_conv_output_dim, embed_dim)  # nn.Linear(256, 200)
        else:
            self.patch_embed_projection = nn.Identity()

        self.patch_embed_seq = nn.Sequential(
            self.patch_embed_conv,
            self.patch_embed_projection
        )
        ...
        if use_abs_pos_emb:
            ...
            self.pos_embed = None
        max_time_patches = PPG_size // self.patch_size if self.patch_size > 0 else 64
        self.time_embed = nn.Parameter(torch.zeros(1, max_time_patches, embed_dim), requires_grad=True)
        print(f"INFO (Pretrain): Adjusted time_embed size to (1, {max_time_patches}, {embed_dim})")
    ...
    def forward_return_qkv(self, x, bool_masked_pos=None, split_out_as_qkv=False):
        ...
        # replace the masked PPG tokens by mask_token
        w = bool_masked_pos.unsqueeze(-1).type_as(mask_token)
        x = x * (1 - w) + mask_token * w
...
class NeuralTransformerForMEM(nn.Module):
    def __init__(self, PPG_size=12000, patch_size=100, in_chans=1, out_chans=8, vocab_size=2048, embed_dim=200, depth=12,
        ...
        super().__init__()
        self.patch_size = patch_size
        self.student = NeuralTransformerForMaskedPPGModeling(PPG_size, patch_size, in_chans, out_chans, vocab_size,
                                                             embed_dim, depth,
    ...
    def forward(self, x, input_chans=None, bool_masked_pos=None, apply_symmetric_masking=True):
        if x.dim() == 3:
            x = rearrange(x, 'B N (A T) -> B N A T', T=self.student.patch_size)
        ...
        if apply_symmetric_masking:
            ...
            # 模式 B: Progressive Masking 模式
            x_rec_sym = None

        return x_rec, x_rec_sym

@register_model
def sigma_pro_patch100_12000_8k_vocab(pretrained=False, **kwargs):
    ...
    drop_rate = kwargs.pop('drop_rate', 0.08)
    attn_drop_rate = kwargs.pop('attn_drop_rate', 0.08)
...
@register_model
def sigma_pro_patch50_12000_8k_vocab(pretrained=False, **kwargs):
    if "num_classes" in kwargs:
        _ = kwargs.pop("num_classes")
    if 'vocab_size' in kwargs:
        vocab_size = kwargs['vocab_size']
